...
class Olap extends CI_Controller{
	...
	public function filter()
	{
		$this->load->helper('url');
		$this->load->helper('form');
		$tanggalstart = $this->input->post('tanggal1');
		$tanggalend = $this->input->post('tanggal2');
		
		//$datetimestart = new DateTime($tanggalstart);
		//$datetimeend = new DateTime($tanggalend);
		
		$datetimestart = new MongoDate(strtotime($tanggalstart));
		$datetimeend = new MongoDate(strtotime($tanggalend));
		...
		$period = new DatePeriod(
			 new DateTime($tanggalstart),
			 new DateInterval('P1D'),
			 new DateTime($tanggalend)
		);
